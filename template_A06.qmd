...
[https://github.com/Jaenney/Lee_Jaenney_201_A06](https://github.com/Jaenney/Lee_Jaenney_201_A06)
...
```{r}
rounded_points <- numeric(length(points_per_game))
names(rounded_points) <- names(points_per_game)

for(player in names(points_per_game)){
  rounded_points[player] <- round(points_per_game[[player]],2)
}
...
```{r}
each_rounded <- function(x){
  if(x < 100){
    base::round(x,digits= -1)
  }else if (x<1000){
    base::round(x,digits= -2)
  }else{
    base::round(x,digits= -3)
  }
}

rounded_mat <- apply(mat_rand, 1:2, each_rounded)
...
```{r}
avg_temp <- sapply(max_temps[,3:14],mean)
avg_temp
```
...
```{r}
temps_CO <- max_temps[max_temps$state == "CO", ]
temps_OH <- max_temps[max_temps$state == "OH", ]
...
```{r}
annual_temps <- apply(temps_CO[,3:14], 1, median)

maximum <- which.max(annual_temps)
minimum <- which.min(annual_temps)

max_year <- temps_CO$year[maximum]
min_year <- temps_CO$year[minimum]

max_temp <- annual_temps[maximum]
min_temp <- annual_temps[minimum]

print(paste0("The maximum median annual temperature of ", max_temp, " occurred in ", max_year, " and the minimum median annual temperature of ", min_temp, " occured in ", min_year, "." ))
```
